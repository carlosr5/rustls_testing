...
use rustls_benchmarking::{start_client, start_server};
...
fn bench_client_rtts(c: &mut Criterion) {
    ...
    // // We start a TLS server here for either client to be able to connect to.
    // start_server();
    ...
    group.bench_function(BenchmarkId::new("rustls client rtt", num_samples), |b| b.iter(|| start_client("webpki".to_string())));
    group.bench_function(BenchmarkId::new("platform verifier client rtt", num_samples), |b| b.iter(|| start_client("rustls-platform-verifier".to_string())));
