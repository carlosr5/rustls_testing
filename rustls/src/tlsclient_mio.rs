...
/**
 * Looks like this is used to read in the arguments from the command line.
 * If this is just used for that, then we can ignore this since the args are passed through environment variables. Keep an eye on this though.
 */
// #[macro_use]
...
use crate::BenchArgs;
use crate::{OwnedTrustAnchor, RootCertStore};
...
impl TlsClient {
    fn new(
        ...
        cfg: Arc<crate::ClientConfig>,
    ) -> Self {
        Self {
            socket: sock,
            closing: false,
            clean_closure: false,
            tls_conn: crate::ClientConnection::new(cfg, server_name).unwrap(),
        }
    }
    ...
    // Replacing reading in from the command line with reading in from the a pre-written message instead
...
// Keeping the USAGE string here for future reference.
// const USAGE: &str = "
...
pub(crate) fn lookup_ipv4(host: &str, port: u16) -> SocketAddr {
...
fn find_suite(name: &str) -> Option<crate::SupportedCipherSuite> {
    for suite in crate::ALL_CIPHER_SUITES {
...
fn lookup_suites(suites: &[String]) -> Vec<crate::SupportedCipherSuite> {
    let mut out = Vec::new();
...
fn lookup_versions(versions: &[String]) -> Vec<&'static crate::SupportedProtocolVersion> {
    let mut out = Vec::new();

    for vname in versions {
        let version = match vname.as_ref() {
            "1.2" => &crate::version::TLS12,
            "1.3" => &crate::version::TLS13,
            _ => panic!(
...
fn load_certs(filename: &str) -> Vec<crate::Certificate> {
    let certfile = fs::File::open(filename).expect("cannot open certificate file");
    let mut reader = BufReader::new(certfile);
    rustls_pemfile::certs(&mut reader)
        .unwrap()
        .iter()
        .map(|v| crate::Certificate(v.clone()))
        .collect()
}

fn load_private_key(filename: &str) -> crate::PrivateKey {
    let keyfile = fs::File::open(filename).expect("cannot open private key file");
    let mut reader = BufReader::new(keyfile);

    loop {
        match rustls_pemfile::read_one(&mut reader).expect("cannot parse private key .pem file") {
            Some(rustls_pemfile::Item::RSAKey(key)) => return crate::PrivateKey(key),
            Some(rustls_pemfile::Item::PKCS8Key(key)) => return crate::PrivateKey(key),
            Some(rustls_pemfile::Item::ECKey(key)) => return crate::PrivateKey(key),
            None => break,
            _ => {}
        }
    }
...
fn apply_dangerous_options(args: &BenchArgs, _: &mut crate::ClientConfig) {
    if args.insecure.unwrap() {
        panic!("This build does not support --insecure.");
    }
}

/// Build a `ClientConfig` from our arguments
fn make_config(args: &BenchArgs) -> Arc<crate::ClientConfig> {
    ...
    let suites = if !args.suite.clone().unwrap().is_empty() {
        lookup_suites(&args.suite.clone().unwrap())
    } else {
        crate::DEFAULT_CIPHER_SUITES.to_vec()
    };

    let versions = if !args.protover.clone().unwrap().is_empty() {
        lookup_versions(&args.protover.clone().unwrap())
    } else {
        crate::DEFAULT_VERSIONS.to_vec()
    };
    ...
    config.key_log = Arc::new(crate::KeyLogFile::new());

    if args.no_tickets.unwrap() {
        config.resumption = config
            .resumption
            .tls12_resumption(crate::client::Tls12Resumption::SessionIdOnly);
    }

    if args.no_sni.unwrap() {
...
/**
 * Encapsulating the main function into here for benchmarking, except to not rely on the command-line arguments and instead rely on the input parameters to start a server.
 * 
