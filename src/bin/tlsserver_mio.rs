...
fn make_config(args: &BenchArgs, run_type: &str) -> Arc<rustls::ServerConfig> {
...
pub(crate) fn run_configured_server(args: BenchArgs, run_type: &str) {
    ...
    if run_type.eq_ignore_ascii_case("webpki") {
    ...
    let mode = if args.cmd_echo.unwrap() {
        ServerMode::Echo
    } else if args.cmd_http.unwrap() {
        ServerMode::Http
    } else {
